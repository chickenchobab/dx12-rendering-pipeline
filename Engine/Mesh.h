...
struct KeyFrameInfo
{
	double	time;
	int32	frame;
	// Make the transform separate for blending.
	Vec3	scale;
	Vec4	rotation;
	Vec3	translate;
};
...
private:
	...
	vector<IndexBufferInfo>		_vecIndexInfo; // Multiple index buffer

	// Animation
	vector<AnimClipInfo>			_animClips;
	vector<BoneInfo>				_bones;

	shared_ptr<StructuredBuffer>		_offsetBuffer; // Offset matrix of each bone
	vector<shared_ptr<StructuredBuffer>> _frameBuffer; // Frame information of all the bones           
};